...
public class GenerateHisVol {
	...
	public List<Map> execute(List<Map> dataList, List<String> matList, String asOfDate, String cutoffDate, String assetType, String name){
		...
		Variant fixingManager = pilCom.invoke("Generic_MktData_FixingManager_Create", fdArrayVar, new Variant(name), new Variant(1));
		log.info("FixingManager created : " + fixingManager);
		
		if(cutoffDate == null) cutoffDate = asOfDate;
		Variant matArrayVar = vp.generateMaturities(matList);
		Variant his_vols = pilCom.invoke("Generic_MktData_HistoricalVolatility_FromFixingManager", fixingManager, new Variant(name), matArrayVar, new Variant("HistoVol"),
				new Variant(), new Variant(), new Variant(), new Variant(), new Variant(), new Variant(), new Variant(du.date2ExcelNumber(cutoffDate)), new Variant(1));
